<script lang="ts">
  ...
  $: currentTheme =
    themes[guestProfile.portfolio_theme as keyof typeof themes] || themes.modern
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Preview of {guestProfile.full_name}'s developer portfolio"
  />
</svelte:head>

<div
  class="container mx-auto px-4 py-8 {guestProfile.portfolio_theme === 'dark'
...
>
  ...
  >
    <div class="flex">
      ...
      <div class="ml-3">
        <p class="text-sm">
          <strong>Preview Mode:</strong> This is how your portfolio will look
          when published.
  ...
  {:else}
    ...
    <div class="{currentTheme.heroClass} rounded-lg p-8 mb-8">
      <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
        <div
          class="w-32 h-32 bg-gray-300 rounded-full flex items-center justify-center"
        >
          <svg
        ...
        <div class="flex-1 text-center md:text-left">
          ...
          <p class="text-lg {currentTheme.subtextClass} mb-4 max-w-2xl">
            {guestProfile.bio ||
              "Your bio will appear here. Add a bio in the editor to see it displayed."}
          </p>
          ...
          <div class="flex justify-center md:justify-start gap-4">
            {#if guestProfile.github_url}
              <a
                href={guestProfile.github_url}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub Profile"
              >
                <div
                  class="w-8 h-8 bg-gray-800 rounded flex items-center justify-center hover:bg-gray-700 transition-colors"
                  aria-label="GitHub Profile"
                >
                  <svg
            ...
            {#if guestProfile.linkedin_url}
              <a
                href={guestProfile.linkedin_url}
                target="_blank"
                rel="noopener noreferrer"
            ...
            {#if guestProfile.twitter_url}
              <a
                href={guestProfile.twitter_url}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Twitter Profile"
              >
                <div
                  class="w-8 h-8 bg-blue-400 rounded flex items-center justify-center hover:bg-blue-500 transition-colors"
                  aria-label="Twitter Profile"
                >
                  <svg
                    class="w-5 h-5 text-white"
    ...
    {#if guestProjects.length === 0}
      <div class="text-center py-12">
        ...
        <a href="/build" class="btn btn-primary mt-4"> Add Projects </a>
      </div>
    {:else}
    ...
    <div class="mt-16 text-center">
      <div class="{currentTheme.cardClass} rounded-lg p-8 shadow-xl">
        ...
        <p class="text-gray-600 mb-6">
          Sign up now to get a live URL and start sharing your portfolio with
          the world!
        </p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <a href="/build" class="btn btn-outline btn-lg"> Continue Editing </a>
        </div>
      </div>
    </div>
  {/if}

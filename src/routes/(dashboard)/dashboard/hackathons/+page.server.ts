...
export const actions: Actions = {
  createHackathon: async ({ request, locals: { supabase, safeGetSession } }) => {
  ...
  updateHackathon: async ({ request, locals: { supabase, safeGetSession } }) => {
    const { session } = await safeGetSession();
    if (!session) {
      throw redirect(303, '/login');
    }
  ...
  deleteHackathon: async ({ request, locals: { supabase, safeGetSession } }) => {
    const { session } = await safeGetSession();
    if (!session) {
      throw redirect(303, '/login');
    }
  ...
  createCertificate: async ({ request, locals: { supabase, safeGetSession } }) => {
    const { session } = await safeGetSession();
    if (!session) {
      throw redirect(303, '/login');
    }
    ...
    const { error: insertError } = await supabase.from('hackathon_certificates').insert({
      ...certificateData,

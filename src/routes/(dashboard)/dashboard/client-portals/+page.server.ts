...
export const actions: Actions = {
  createClientPortal: async ({ request, locals: { supabase, safeGetSession } }) => {
    ...
    const { error: insertError } = await supabase.from('client_portals').insert({
      ...portalData,
  ...
  updateClientPortal: async ({ request, locals: { supabase, safeGetSession } }) => {
    const { session } = await safeGetSession();
    if (!session) {
      throw redirect(303, '/login');
    }
  ...
  deleteClientPortal: async ({ request, locals: { supabase, safeGetSession } }) => {
    const { session } = await safeGetSession();
    if (!session) {
      throw redirect(303, '/login');
    }

...
<svelte:head>
  ...
  <meta
    name="description"
    content="Track your portfolio progress, earn achievements, and see how you rank!"
  />
</svelte:head>
  ...
  <div class="grid gap-4 md:grid-cols-3">
    <Card>
      ...
      <CardContent>
        <p class="text-4xl font-bold">
          {gamificationData?.portfolioScore?.total_score || 0}
        </p>
      </CardContent>
    </Card>
    <Card>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription
          >{gamificationData?.userAchievements?.length || 0} / {gamificationData
            ?.achievements?.length || 0} earned</CardDescription
        >
      </CardHeader>
      <CardContent>
        <p class="text-4xl font-bold">
          {Math.round(
            ((gamificationData?.userAchievements?.length || 0) /
              (gamificationData?.achievements?.length || 1)) *
              100,
          )}%
        </p>
      </CardContent>
      ...
      <CardContent class="pt-6">
        <div class="flex justify-between items-center mb-2">
          ...
          <span class="text-lg font-bold"
            >{gamificationData?.completionPercentage || 0}%</span
          >
        </div>
        <Progress
          value={gamificationData?.completionPercentage || 0}
          class="w-full"
        />
        <p class="text-sm text-muted-foreground mt-2">
          Complete your profile and add projects to reach 100%!
        </p>
      </CardContent>
    </Card>
  </div>
      ...
      <CardContent class="pt-6">
        {#if gamificationData?.userAchievements && gamificationData.userAchievements.length > 0}
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {#each gamificationData.userAchievements as userAchievement}
              <div
                class="flex flex-col items-center text-center p-4 border rounded-lg"
                title={userAchievement.achievements.description}
              >
                ...
                <p class="text-xs text-muted-foreground">
                  +{userAchievement.achievements.points} pts
                </p>
              </div>
            {/each}
          </div>
        {:else}
          <p class="text-center text-muted-foreground py-8">
            You haven't unlocked any achievements yet. Keep building your
            portfolio!
          </p>
        {/if}
      </CardContent>

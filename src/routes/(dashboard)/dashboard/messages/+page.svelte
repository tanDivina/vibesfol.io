...
<div class="container mx-auto px-4 py-8">
  <div class="flex justify-between items-center mb-8">
    <div>
      ...
      <p class="text-gray-600 mt-2">Manage contact form submissions from your portfolio</p>
    </div>
  </div>
  ...
  <div class="bg-white rounded-lg shadow mb-6">
    <div class="border-b border-gray-200">
      <nav class="-mb-px flex space-x-8 px-6">
        <button
          class="py-4 px-1 border-b-2 font-medium text-sm {selectedFilter === 'all' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          on:click={() => selectedFilter = 'all'}
        >
          All Messages ({data.stats.total})
        </button>
        <button
          class="py-4 px-1 border-b-2 font-medium text-sm {selectedFilter === 'unread' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          on:click={() => selectedFilter = 'unread'}
        >
          Unread ({data.stats.unread})
        </button>
        <button
          class="py-4 px-1 border-b-2 font-medium text-sm {selectedFilter === 'starred' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          on:click={() => selectedFilter = 'starred'}
        >
          Starred ({data.stats.starred})
        </button>
        <button
          class="py-4 px-1 border-b-2 font-medium text-sm {selectedFilter === 'replied' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
    ...
    <div class="divide-y divide-gray-200">
      ...
      {:else}
        {#each filteredMessages as message (message.id)}
          <div class="p-6 hover:bg-gray-50 cursor-pointer {!message.is_read ? 'bg-blue-50' : ''}" on:click={() => openMessage(message)}>
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center space-x-2">
                      ...
                      {#if message.sender_company}
                        <span class="text-xs text-gray-500">at {message.sender_company}</span>
                      {/if}
                      {#if !message.is_read}
                    ...
                    <p class="text-sm text-gray-900 font-medium mt-1">{message.subject}</p>
                    <p class="text-sm text-gray-500 truncate mt-1">{message.message}</p>
                  </div>
                </div>
              </div>
...
{#if selectedMessage}
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" on:click={closeMessage}>
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white" on:click|stopPropagation>
      <div class="flex justify-between items-start mb-4">
        <div class="flex-1">
          ...
          <p class="text-xs text-gray-400 mt-1">Received: {formatDate(selectedMessage.created_at)}</p>
        </div>
        <button class="text-gray-400 hover:text-gray-600" on:click={closeMessage}>
      ...
      <div class="flex justify-between items-center">
        <div class="flex space-x-2">
          ...
          <form method="POST" action="?/markAsReplied" use:enhance={() => {
            return async ({ result }) => {
        ...
        <div class="flex space-x-2">
          <form method="POST" action="?/toggleStar" use:enhance={() => {
            ...
            <input type="hidden" name="messageId" value={selectedMessage.id} />
            <input type="hidden" name="isStarred" value={selectedMessage.is_starred} />
            <button type="submit" class="btn btn-ghost">
              <svg class="w-4 h-4 {selectedMessage.is_starred ? 'text-yellow-400' : 'text-gray-400'}" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
              </svg>
            </button>
          </form>

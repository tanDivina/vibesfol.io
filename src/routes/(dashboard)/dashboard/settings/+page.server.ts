...
export const actions: Actions = {
  ...
  updateContactFormSettings: async ({ request, locals: { safeGetSession } }) => {
    const { session, user } = await safeGetSession();
    ...
    const contact_form_description = formData.get('contact_form_description') as string;
    const contact_email_notifications = formData.get('contact_email_notifications') === 'on';
    ...
    return { success: true, message: 'Contact form settings updated successfully' };
  },
  ...
  updateCustomDomain: async ({ request, locals: { safeGetSession } }) => {
    ...
    if (error) {
      ...
      return fail(500, { message: 'Failed to update custom domain. It might already be in use.' });
    }

    return { success: true, message: 'Custom domain updated successfully. Please verify the new domain.' };
  },
